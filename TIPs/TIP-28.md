...
This TIP proposes a format for Optimisticly created and resolved Positional Markets.
 
## Abstract
 
It's the vision of the Thales Protocol to support exotic positional markets that anyone can create. This TIP proposes a format that can achieve this in a decentralized manner with an optimistic type of oracle overviewed by an Oracle Council voted in by Thales Stakers. 
 
## Motivation
 
Since inception Thales set out to become a parimutuel protocol where individual users can chip-in and take a position. The said positions have to be mutually exclusive and capture a full range or potential outcomes.  
Initially only binary positions were supported, but the framework proposed here allows for any number of positions to be defined, as long as the adhere to the rules of being mutually exclusive and capturing all outcomes.  
...
However, it is fully expected that to meet the demand Thales needs another framework that allows for more flexible ad-hoc markets.  

The proposed framework intents to capture such markets and is quite flexible in terms of possibilites it could lead to.
...
Thales Treasury DAO will fund Oracle Council multisig wallet with 10,000 THALES budget to be used to rewards successful disputes.
...
Oracle Council commits to resolve disputes within 24h, otherwise the council as a whole or individial members could be relieved at the discretion of Thales council.  
...
- A list of positions participants can assume (up to 5 positions supported of type String)
...
As soon as a market is created its in positioning phase and open to participants.
...
If allowed, withdrawals are subject to a total 6% fee. 3% paid to the market creator and 3$ towards Safe Box. These fees are pain within the withdrawal transactions.  
...
Every market must be created according to the following guidelines. Not adhering to these guidelines can be a subject of dispute:
...
For `Open Bid` market one wallet can enter multiple bids and the spread the bids across multiple positions. To reposition, they have to first pull the bid from a position and then choose a new deposit it into a new position.

### Disputes in positioning phase
Anyone can dispute a market while in positioning phase. To open a dispute the wallet raising the dispute needs to put up 100 THALES bond.
On raising the dispute, the person disputing the market inputs the reason for the dispute as string (up to 100 characters).  
There can be at most 1 active dispute per market. 
...
In addition to all positions created with the market, there is a default position available to select when resolving a market which is called `Refund`.  
...
If 24h has passed since the market was resolved and there are no open disputes, the market is formally resolved.
